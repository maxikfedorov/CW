...
switch ($requestMethod) {
    case 'GET':
        if (empty(isset($_GET['class_id']))) {
            $result = $con->query("SELECT * FROM item_class;");

            while ($row = $result->fetch_assoc()) {
                $answer[] = $row;
            }
        } else {
            $query_result = $con->query("SELECT * FROM item_class WHERE class_id = " . $_GET['class_id'] . ";");
    ...
    case 'POST':
        ...
        if (!empty($client->{'class_descr'})) {
            $class_descr = $client->{'class_descr'};

            #$query_result = $con->query("SELECT * FROM clients WHERE client_surname = '" . $client_surname . "'");

            $stmt = $con->prepare("INSERT INTO item_class (class_descr) VALUES (?)");
            $stmt->bind_param('s', $class_descr);
            $stmt->execute();
...
*/
        ...
        else
        {
            ...
            if (!empty($result)){
                ...
                if(!empty($obj->{'class_descr'}))
                    $con->query("UPDATE item_class SET class_descr='".$obj->{'class_descr'}."'
    ...
    case 'DELETE':
        if (empty(isset($_GET['class_id']))) {
            ...
            $result = $query_result->fetch_row();
            if (!empty($result)) {
                $query_result = $con->query("DELETE FROM item_class WHERE class_id='" . $_GET['class_id'] . "'");
